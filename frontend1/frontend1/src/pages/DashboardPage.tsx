...
const formatPrice = (price: number): string => {
  if (price >= 1000) {
    return `${(price / 1000).toFixed(1)}K F`;
  }
...
const DashboardPage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const chartData = transactions
    .slice(0, 7)
    .reverse()
    .map((tx, index) => ({ jour: `J-${6 - index}`, transactions: 1, revenus: tx.fraisService || 0 }));
  ...
  return (
    <div style={{ animation: 'fadeUp 0.4s ease' }}>
      ...
      <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 16 }}>
        <StatCard label="Transactions" value={loading || !stats ? '...' : stats.totalTransactions.toLocaleString('fr')} change={12.5} icon={<ArrowLeftRight size={18} />} color="var(--primary)" />
        <StatCard label="Revenus du jour" value={loading || !stats ? '...' : formatPrice(stats.revenusJour)} change={8.2} icon={<Wallet size={18} />} color="var(--success)" />
        <StatCard label="Revenus du mois" value={loading || !stats ? '...' : formatPrice(stats.revenusMois)} change={-3.1} icon={<TrendingUp size={18} />} color="var(--warning)" />
        <StatCard label="Clients actifs" value={loading || !stats ? '...' : stats.totalClients.toLocaleString('fr')} change={5.7} icon={<Users size={18} />} color="#a78bfa" />
      </div>

      {/* Monnaie distribuée / récupérée */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        <div className="card" style={{ textAlign: 'center', padding: '14px' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 800, color: 'var(--primary)' }}>{loading || !stats ? '...' : formatPrice(stats.monnaieDistribuee)}</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 3 }}>Monnaie distribuée</div>
        </div>
        <div className="card" style={{ textAlign: 'center', padding: '14px' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 800, color: 'var(--success)' }}>{loading || !stats ? '...' : formatPrice(stats.monnaieRecuperee)}</div>
      ...
      <div className="chart-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12, marginBottom: 16 }}>
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={chartData.length ? chartData : [{ jour: 'J-6', transactions: 0, revenus: 0 }, { jour: 'J-5', transactions: 0, revenus: 0 }, { jour: 'J-4', transactions: 0, revenus: 0 }, { jour: 'J-3', transactions: 0, revenus: 0 }, { jour: 'J-2', transactions: 0, revenus: 0 }, { jour: 'J-1', transactions: 0, revenus: 0 }, { jour: 'Aujourd\'hui', transactions: 0, revenus: 0 }]} margin={{ top: 0, right: 0, left: -30, bottom: 0 }}>
              <defs>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={chartData.length ? chartData : [{ jour: 'J-6', transactions: 0, revenus: 0 }, { jour: 'J-5', transactions: 0, revenus: 0 }, { jour: 'J-4', transactions: 0, revenus: 0 }, { jour: 'J-3', transactions: 0, revenus: 0 }, { jour: 'J-2', transactions: 0, revenus: 0 }, { jour: 'J-1', transactions: 0, revenus: 0 }, { jour: 'Aujourd\'hui', transactions: 0, revenus: 0 }]} margin={{ top: 0, right: 0, left: -30, bottom: 0 }}>
              <XAxis dataKey="jour" tick={{ fontSize: 10, fill: 'var(--text-muted)' }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 9, fill: 'var(--text-dim)' }} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
