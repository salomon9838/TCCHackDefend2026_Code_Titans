...
const PartnerScanPage: React.FC = () => {
  ...
  const [qrData, setQrData] = useState<{ ref: string; montant: number; emetteur: string; clientNom: string; expires: string; score: number } | null>(null);
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    setScanResult('scanning');
    try {
      const result = await scanQr(manualRef.trim());
      if (result.status === 'valid') {
        setScanResult('valid');
        setQrData({
          ref: result.reference ?? manualRef,
          montant: result.montant ?? 0,
          emetteur: 'SmartChange Client',
          clientNom: 'Client SmartChange',
          expires: result.expiration ?? new Date(Date.now() + 48 * 3600000).toISOString(),
          score: result.isFraud ? 87 : 18,
        });
      ...
      {scanResult === 'valid' && (
        <div style={{ animation: 'fadeUp 0.3s ease' }}>
          <div className="card" style={{ border: '1px solid var(--success)', marginBottom: 16 }}>
            ...
            {[
              { label: 'Référence', value: qrData?.ref ?? mockQrData.ref, mono: true },
              { label: 'Client', value: qrData?.clientNom ?? mockQrData.clientNom },
              { label: 'Émetteur', value: qrData?.emetteur ?? mockQrData.emetteur },
              { label: 'Montant à rembourser', value: `${(qrData?.montant ?? mockQrData.montant).toLocaleString('fr')} F CFA`, bold: true, color: 'var(--warning)' },
              { label: 'Score de risque IA', value: `${qrData?.score ?? 18}/100 — ${((qrData?.score ?? 18) < 40 ? '✅ Faible' : (qrData?.score ?? 18) < 70 ? '⚠️ Modéré' : '🚨 Élevé')}`, color: (qrData?.score ?? 18) < 40 ? 'var(--success)' : (qrData?.score ?? 18) < 70 ? 'var(--warning)' : 'var(--danger)' },
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button className="btn btn-success btn-lg" style={{ flex: 2 }} onClick={handleValidate} disabled={loading}>
                {loading ? <div className="spinner" /> : <><CheckCircle size={18} /> Rembourser {(qrData?.montant ?? mockQrData.montant).toLocaleString('fr')} F CFA</>}
              </button>
            </div>
          </div>
        </div>
