...
const TransactionPage: React.FC = () => {
  ...
  const [transactionId, setTransactionId] = useState(`TXN-${Date.now().toString().slice(-6)}`);
  ...
  const generateQR = async (reference: string) => {
    const data = JSON.stringify({ ref: reference, montant: monnaie, emetteur: 'Boutique Kokou', expires: new Date(Date.now() + 48 * 3600000).toISOString() });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Paiement Fadadey non disponible', paymentError);
      }

      if (monnaie > 5000) setShowFraudWarning(true);
      await generateQR(response.transactionId);
      setStep('qr');
    } catch (error) {
      console.error('Erreur création transaction', error);
      alert('Impossible de créer la transaction. Vérifiez votre connexion et réessayez.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={{ animation: 'fadeUp 0.3s ease' }}>
          <div className="card" style={{ marginBottom: 14 }}>
            ...
            <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-sm)', padding: '12px 14px', textAlign: 'center', marginBottom: 14 }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--primary)', marginBottom: 4 }}>{createdTransaction?.transactionId ?? transactionId}</div>
      ...
      {step === 'qr' && (
        <div style={{ animation: 'fadeUp 0.3s ease' }}>
          <div className="card" style={{ textAlign: 'center', marginBottom: 14 }}>
            ...
            {paymentUrl && (
              <div style={{ background: 'var(--primary-dim)', border: '1px solid var(--primary)', borderRadius: 'var(--radius-sm)', padding: '12px 14px', marginBottom: 14, color: 'var(--primary)', fontSize: 13, textAlign: 'left' }}>
                Paiement Fadadey initié. Un nouvel onglet s'est ouvert pour finaliser la transaction.
              </div>
            )}
