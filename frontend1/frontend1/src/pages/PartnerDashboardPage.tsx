...
const formatPrice = (price: number | string): string => {
  ...
  if (num >= 1000) {
    return `${(num / 1000).toFixed(1)}K F`;
  }
  return `${num.toLocaleString('fr')} F`;
};
...
const PartnerDashboardPage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const chartData = transactions.length > 0
    ? transactions.slice(-7).map((tx, index) => ({
        jour: `J-${6 - index}`,
        remboursements: tx.monnaieARendre,
        commissions: tx.fraisService,
      }))
    : partnerChartDataFallback;

  const recentReimbursements = transactions.slice(0, 5).map(tx => ({
    ref: tx.transactionId,
    montant: tx.montantPaye,
    commission: tx.fraisService,
    heure: new Date(tx.createdAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
    statut: tx.statut,
  }));
  ...
  const partnerStats = {
    ...
    commissionsMois: stats?.revenusMois ?? 0,
    ...
    <div style={{ animation: 'fadeUp 0.4s ease' }}>
      ...
      <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 16 }}>
        <StatCard
          ...
          value={formatPrice(partnerStats.commissionsGagnees)}
          change={8.2}
          icon={<Coins size={18} />}
          color="#a78bfa"
        />
        <StatCard
          ...
          value={formatPrice(partnerStats.commissionsMois)}
          change={14.3}
        ...
        <StatCard
          ...
          value={formatPrice(partnerStats.monnaieRemboursee)}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        <div className="card" style={{ textAlign: 'center', padding: '14px', background: 'linear-gradient(135deg, var(--primary-dim), transparent)' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 800, color: 'var(--primary)' }}>
            {formatPrice(partnerStats.monnaieRemboursee)}
          </div>
        ...
        <div className="card" style={{ textAlign: 'center', padding: '14px', background: 'linear-gradient(135deg, rgba(167,139,250,0.12), transparent)' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 800, color: '#a78bfa' }}>
            {formatPrice(partnerStats.commissionsGagnees)}
          </div>
