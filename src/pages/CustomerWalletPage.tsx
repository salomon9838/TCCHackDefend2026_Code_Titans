...
const CustomerWalletPage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const balance = 670;
  const credits = [
    { ref: 'TXN-002', montant: 250, merchant: 'Marché Central', expires: '2024-01-17', statut: 'actif' },
    { ref: 'TXN-003', montant: 800, merchant: 'Épicerie Akosua', expires: '2024-01-17', statut: 'actif' },
    { ref: 'TXN-006', montant: 380, merchant: 'Pharmacie Kpalimé', expires: '2024-01-15', statut: 'expiré' },
  ];
    ...
    <div style={{ animation: 'fadeUp 0.4s ease' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
          <Wallet size={14} /> Solde total disponible
        </div>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 44, fontWeight: 900, color: 'white' }}>
          {balance.toLocaleString('fr')} <span style={{ fontSize: 20, color: 'var(--success)', fontWeight: 400 }}>F CFA</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            { label: 'Crédits actifs', value: '2', color: 'var(--success)' },
            { label: 'Total reçu', value: '1 430 F', color: 'var(--primary)' },
            { label: 'Total utilisé', value: '760 F', color: 'var(--warning)' },
          ].map((s, i) => (
